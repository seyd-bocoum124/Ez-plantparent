...
@Component({
  selector: 'app-express-analysis',
  standalone: true,
  imports: [CommonModule, MatButton, MatChipListbox, MatChip, ExpressLiveChartComponent, MatStepper, MatStep, MatStepLabel, MatStepperNext, MatProgressBar, RouterLink],
  templateUrl: './component.html',
  styleUrls: ['./component.scss'],
  changeDetection: ChangeDetectionStrategy.OnPush
})

export class ExpressAnalysisComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  constructor(
    ...
    effect(() => {
      ...
      if (stationId) {
        console.log('[ExpressAnalysis] Station changed to:', stationId);
  ...
  // Expose le signal pour utilisation dans le template
  ...
  ngOnInit(): void {
    ...
    console.debug('[ExpressAnalysis] ngOnInit - WebSocket will connect via effect when stationId is available');
      ...
      next: (msg) => {
        ...
        if(_.get(msg, "type") === "data") {
          ...
          this.seriesService.addPoints(
            _.get(data, "humidity"),
            _.get(data, "air_humidity"),
            _.get(data, "temperature")
          )
          ...
          this.cdr.markForCheck();
        } else if(JSON.stringify(_.pick(msg, ["type", "action", "activity"])) === JSON.stringify({"type":"command","action":"present_report", "activity": "express_analysis"})) {
          const report_id = _.get(msg, "data.report_id");
          this.stationService.getExpressAnalysisReportById(report_id).subscribe((data: any) => {
            this.expressAnalysisReport.set(data);
            const report = {
              soil_humidity_data: _.get(data, "soil_humidity_data", []),
              air_humidity_data: _.get(data, "air_humidity_data", []),
              temperature_data: _.get(data, "temperature_data", [])
            };
  ...
  async deleteReport() {
    ...
    try {
      await this.stationService.deleteExpressAnalysisReport(reportId);
  ...
  cmdExpressAnalysis(action: string): void {
